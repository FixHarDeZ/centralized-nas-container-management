...
- **Line Secretary** — FastAPI + Python 3.12 personal AI secretary LINE bot backed by Notion. Stateless (no volume needed). Uses OpenAI-compatible API — configurable to Groq (`AI_PROVIDER=groq`, free tier) or OpenRouter (`AI_PROVIDER=openrouter`, pay-per-use). On every message, runs Notion keyword search and a parallel shallow header scan of all pages, then merges results — ensures content inside nested toggle blocks (not indexed by Notion's search API) is still found. Supports reading simple tables, toggle blocks, and embedded databases. Write operations (Notion simple tables and proper databases) go through a confirmation step. Port 5057 internally; external HTTPS via Synology Reverse Proxy on port 5058.
