...
| `claude-desk/` | Claude Code as a document desk from the phone — web terminal + key bar, pptx/docx/xlsx skills, `out/` download drawer, sessions sheet (tap to resume), 5h/7d rate-limit chip, workstation status line (`STATUSLINE_BAR_W` picks the phone layout) | `5072` (Nginx basic-auth) | `https://…:15072` |
