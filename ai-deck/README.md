...
**MiMo in Chat** (document mode only — the coding worker has no mimo key, so the option is hidden there and `chat.py` answers 400). Choosing **MiMo** in the Agent menu runs one `mimo run --format json -s <session> -- <text>` child per turn (`mimo_backend.py`, same shape as Codex), with stdin closed because `run` appends a non-TTY stdin to the message. MiMoCode emits whole text parts, not deltas, so answers land a part at a time; tool calls arrive already finished. A model error comes back as an `error` event with **exit code 0** — that event, not the exit code, marks the turn failed. Sessions and history come from MiMoCode's own SQLite store (`~/.local/share/mimocode/mimocode.db`, opened read-only), filtered to this workspace's directory, top-level sessions only, and without the Claude Code transcripts MiMoCode imports into the same table. `-s` resumes a session from any cwd, so the workspace check is ours. Model menu is read from `mimocode.jsonc` itself (add a model there, it shows up; the endpoint's `/models` also lists ASR/TTS models that are not chat models, so it is not used). Effort offers MiMoCode's own variants `low`/`medium`/`high`, passed as `mimo run --variant`; Default sends no variant and keeps the config's pinned `reasoningEffort: low`. Quota chip `mo 12.3M ↻4d` is a **local estimate**: the Token Plan key has no usage endpoint (only the web console, behind a ~24h login cookie), so `usage_status.mimo_status()` sums `tokens.total` of every mimo `step-finish` part this calendar month from MiMoCode's DB — chat and Terminal both, but not `ask`, other stacks on the same key, or deleted sessions. % is against `MIMO_MONTHLY_TOKEN_LIMIT` (default 4.1B, a figure another project hardcoded, not Xiaomi's).
...
- **Claude:** The header reads `get_usage` through the installed CLI control protocol, without a model turn (60-second cache, 15-second manual-refresh cooldown). Chat, Terminal and metadata use the saved Claude login in their worker home; the legacy environment setup-token no longer overrides it. Run `claude auth login` in each worker's Terminal if needed; document and coding homes are separate. Chat also consumes the CLI's `rate_limit_event`, including `unifiedWindows` in the pinned CLI, and pushes a `quota` SSE event immediately. Per-window snapshots are atomically saved in `~/.claude/desk-chat-status.json`. `GET /api/status?provider=claude` merges the newest observation for each window with Terminal's `~/.claude/desk-status.json`, preserving each window's own age. Starting Chat no longer depends on a Terminal status-line render.
...
- A missing observation shows **Quota pending**. A failed refresh shows **Quota unavailable**. After 20 minutes without an observation, or once the observed window has expired, its percentage becomes **— stale**, with the old reading available in the tooltip. The client never invents 0% at reset and never makes a stale weekly reading fresh because only the 5-hour window updated.
...
- `stacks.claude_desk.oauth_token` → `CLAUDE_CODE_OAUTH_TOKEN` is retained as legacy vault configuration, but the desk clears this override at startup. Claude now uses its saved `claude auth login` credentials so Chat and quota share the same account and profile access. No API key is required.
...
- **Connection status follows the active view.** Chat uses its SSE connection for `live`/`reconnecting`; Terminal uses its WebSocket. Opening or switching the Chat provider does not attach a hidden terminal. Terminal connects when opened; Start/Login waits for terminal output before issuing its command. Slow quota/model reads do not hold the connection indicator.
- **Claude model versions** come from the installed CLI initialize response (`resolvedModel`), along with its supported effort levels; aliases remain the selection values. If metadata is unavailable, fallback aliases explicitly say the version is unavailable.
- **Quota requires provider observations.** Claude reads usage metadata while idle and also updates after response headers arrive; a new empty chat alone does not consume quota. Codex account reads can fail or lack subscription windows. Missing/old data is visibly marked, never estimated from tokens.
