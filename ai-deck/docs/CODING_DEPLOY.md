...
The LaunchAgent is `~/Library/LaunchAgents/local.ai-deck.deploy-runner.plist`. It runs the daemon after user login with `KeepAlive`, explicit PATH/HOME, operator-owned profiles/data/token paths, and `--tls-cert` / `--tls-key`. The listener binds `127.0.0.1`; its TLS certificate includes that loopback IP. Renew it before expiry. The document backend connects through the SSH socket tunnel below, so a changing Mac LAN address does not affect deployment. The daemon checks client bearer authentication before accepting any asserted desk user.
