...
(function () {
  ...
  // ── Rate-limit chip ───────────────────────────────────
  // Chat events and native account metadata update each provider separately.
  // Keep each window's age; expired observations must not look current.
  ...
  function fillWindow(row, win, label, data) {
  ...
  function renderQuota(data) {
    ...
    lastQuota = data;
    const tips = [
      fillWindow(quota.querySelector('.q-5h'), data.five_hour, '5h', data),
      fillWindow(quota.querySelector('.q-wk'), data.seven_day, 'wk', data),
    ].filter(Boolean);
  ...
  async function loadQuota(force = false) {
    ...
    try {
      const r = await fetch(deskURL('api/status' + (force ? '?refresh=1' : '')), { cache: 'no-store' });
      if (!r.ok) throw new Error('quota unavailable');
      const data = await r.json();
      if (version === quotaVersion) renderQuota(data);
      if (provider === 'claude') noteFinished(data.done);
    } catch (_) {
      if (version === quotaVersion) renderQuota({ ...(lastQuota || {}), status: 'unavailable' });
    } finally { quotaPending = false; }
  }
  quota.addEventListener('click', () => loadQuota(true));
  loadQuota();
  setInterval(() => { if (lastQuota) renderQuota(lastQuota); }, 15000);
  setInterval(() => { if (!document.hidden) loadQuota(); }, 20000);
  ...
  function noteFinished(done) {
    ...
    try {
      ...
      new Notification('AI Deck', { body: 'Claude finished.', tag: 'claude-desk-done' });
  ...
  function emptyState() {
    ...
    box.innerHTML = '<span class="welcome-kicker">A LITTLE SPACE TO MAKE THINGS</span>' +
      ...
      '<span aria-hidden="true">▧</span><strong>Build a presentation</strong><small>From source files to a clear story</small></button>' +
      '<button type="button" data-prompt="ช่วยอ่านไฟล์ใน in/ แล้วสรุปประเด็นสำคัญและสิ่งที่ควรทำต่อ">' +
      '<span aria-hidden="true">≡</span><strong>Make sense of a document</strong><small>Find the details that matter</small></button>' +
  ...
  function applyChatEvent(ev) {
    if (ev.t === 'quota') { quotaVersion += 1; renderQuota(ev.data); return; }
  ...
  function setView(which) {
    ...
    viewChatBtn.setAttribute('aria-selected', String(chatOn));
    viewTermBtn.setAttribute('aria-selected', String(!chatOn));
    ...
    if (chatOn) {
      ...
      if (!DEMO) {
        fetch(deskURL('chat/state'), { cache: 'no-store' })
  ...
  providerSelect.addEventListener('change', () => {
    saveDraft();
    try { localStorage.setItem('ai-desk.provider', providerSelect.value); } catch (_) {}
  ...
  async function refreshAuth() {
    ...
    try {
      const response = DEMO ? null : await fetch(deskURL('api/auth'), { cache: 'no-store' });
  ...
  const optionsNote = document.getElementById('chat-options-note');
  const PREFS_KEY = 'ai-deck.chat-options.' + provider;
  let modelOptions = [], optionsPending = false, optionsLoaded = false;
  function saveOptions() {
    try { localStorage.setItem(PREFS_KEY, JSON.stringify({ model: modelSelect.value, effort: effortSelect.value })); } catch (_) {}
  }
  function paintEfforts(wanted = '') {
    const model = modelOptions.find((item) => item.id === modelSelect.value);
    const levels = model ? model.efforts : [];
    effortSelect.replaceChildren(new Option('Default', ''));
    levels.forEach((level) => effortSelect.add(new Option(level, level)));
    effortSelect.value = levels.includes(wanted) ? wanted : '';
    effortSelect.disabled = levels.length === 0;
  }
  async function refreshOptions() {
    if (optionsPending) return;
    optionsPending = true;
    let wanted = { model: modelSelect.value, effort: effortSelect.value };
    if (!optionsLoaded) {
      try { wanted = JSON.parse(localStorage.getItem(PREFS_KEY)) || wanted; } catch (_) {}
    }
    try {
      const response = DEMO ? null : await fetch(deskURL('chat/options'), { cache: 'no-store' });
      if (response && !response.ok) throw new Error('models unavailable');
      const data = DEMO ? { models: [
        { id: '', label: 'Default', efforts: [] },
        { id: provider === 'codex' ? 'demo-model' : 'sonnet', label: provider === 'codex' ? 'Demo model' : 'Sonnet', efforts: ['low', 'medium', 'high'] },
      ] } : await response.json();
      ...
      modelSelect.disabled = modelOptions.length < 2;
      optionsNote.textContent = modelOptions.length < 2 ? 'Model list unavailable. Default remains available.' : 'Applies to your next message';
      optionsLoaded = true;
    } catch (_) {
      optionsNote.textContent = 'Could not refresh models. Try returning to this tab.';
    } finally { optionsPending = false; }
  }
  modelSelect.addEventListener('change', () => { paintEfforts(effortSelect.value); saveOptions(); });
  effortSelect.addEventListener('change', saveOptions);
  const optionsReady = refreshOptions();
  refreshAuth();
  window.addEventListener('focus', () => { refreshAuth(); refreshOptions(); });
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) { refreshAuth(); refreshOptions(); }
  });
  ...
  async function openAgent(path, button) {
    ...
    try {
      ...
      if (button === loginButton) refreshAuth();
    }
  }
  loginButton.addEventListener('click', () => {
    if (authStatus === 'signed_out') openAgent('api/login', loginButton);
    else refreshAuth();
  });
  const startButton = document.getElementById('agent-start');
  startButton.addEventListener('click', () => openAgent('api/new', startButton));

  // ── Go ────────────────────────────────────────────────
  if (DEMO) {
    setState('connected', 'live');
    term.writeln('\x1b[38;5;214m▌\x1b[0m AI Deck — 2.1.272 (Claude Code)');
  ...
  function demoChat() {
    clearLog();
    chatLog.querySelector('.chat-empty')?.remove();
    chatState.textContent = providerName + ' · Your files and conversation, in one place';
