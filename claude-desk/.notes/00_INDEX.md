...
- `mimo-code` (`mimo-code.sh` → `/usr/local/bin/mimo-code`, config `/opt/claude-desk/opencode.json`): opencode (`ARG OPENCODE_VERSION`, npm global) with mimo as the model — a real tool loop that reads, writes and runs things in `/work`. `reasoningEffort: low` (camelCase — the snake_case spelling is dropped silently; checked against a local echo server), `limit.output` 32k leaving as `max_tokens`, `instructions` pointed at `/work/CLAUDE.md`, all permissions allowed, `CLAUDE_CODE_OAUTH_TOKEN` unset before exec. Verified end to end on the NAS 2026-09-15 (tool call + bash in the probe, `mimo-code run` after deploy). Deliberately in this stack, not a `mimo-desk` of its own: `ttyd -m 1` + one tmux session means the two agents can never run concurrently anyway.
- `ask.py` → `/usr/local/bin/mimo` in the image: one prompt, one answer on stdout (stdlib urllib, no `openai` package). argv and/or stdin; seconds counter on stderr; no `max_tokens`, `reasoning_effort=low`, `MIMO_ASK_TIMEOUT_SECONDS` (300) deadline, no streaming, no retry. **Not an agent** — mimo driving Claude Code's loop was probed 2026-09-15 and rejected on latency (~28s for a context-free tool call; the endpoint does support `tool_calls` and `max_tokens`), not on capability.
