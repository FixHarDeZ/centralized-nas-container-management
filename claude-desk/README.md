...
- **Copy / Paste** are both buttons on the key bar. Copy is not optional decoration: xterm paints the screen, so a selection inside it is not a DOM selection and the browser's own Cmd/Ctrl+C has nothing to take — without the button, text goes *into* the desk and never out of it. Select, tap **⧉ Copy**; the button reports back (`✓ copied`, `select first`, `blocked`).
