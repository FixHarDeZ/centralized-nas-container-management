...
All three Docker Compose stacks that mount host paths hardcode `/volume1` directly in their YAML. Migrating from `volume1` to `volume2` requires hunting through multiple files. The `.env` already has `NAS_TARGET_PATH=/volume1/docker` for `deploy.sh`, but compose files do not use it.
...
NAS_TARGET_PATH=/volume1/docker  ← also update this (deploy.sh uses it directly)
```
...
| File | Change |
|---|---|
| `uptime-kuma/docker-compose.yml` | `/volume1/docker/uptime-kuma` → `${NAS_VOLUME_ROOT}/docker/uptime-kuma` |
