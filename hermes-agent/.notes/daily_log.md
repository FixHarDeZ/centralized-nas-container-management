...
## 2026-07-02 — Fix dashboard crash loop (v2026.7.1 auth gate)

### Root Cause

`HERMES_REF` bumped to `v2026.7.1` — upstream changed dashboard auth policy: binding to `0.0.0.0` now **requires** a registered auth provider. The `--insecure` flag alone no longer bypasses this. Container crash-looped with:
...
- Created `scripts/inject-dashboard-auth.sh` — reads `DASHBOARD_PASSWORD_HASH` + `DASHBOARD_BASIC_AUTH_USER` env vars, appends `dashboard.basic_auth` block to `config.yaml` on volume before s6 starts (idempotent: skips if already present)
- Updated `docker-compose.yml`: added env vars, mounted script, changed entrypoint to `["/opt/inject-dashboard-auth.sh", "/init", "/opt/hermes/docker/main-wrapper.sh"]`
- Added `DASHBOARD_PASSWORD_HASH` to `secrets.manifest.yaml` → vault key `stacks.hermes_agent.dashboard.password_hash`
- Password + hash generated by user via `make edit-vault`

### Deploy

- `make secrets` + `deploy.sh -s hermes-agent -y` — all 3 containers started without crash loop

### Update: Remove nginx sidecar

Since hermes now has built-in basic auth (v2026.7.1+), nginx sidecar is redundant:
- Removed `nginx` service from `docker-compose.yml`
- Changed dashboard port mapping: `expose: 9119` → `ports: 5063:9119`
- Removed `--insecure` flag (no longer needed with auth configured)
- Updated `README.md`, `CLAUDE.md`, `.notes/00_INDEX.md`
