...
- **(2026-08-18) `bank_name` / `bank_account` / `bank_account_name`** บน `employees` — ใช้เมื่อ `payment_method='transfer'` เท่านั้น (ฟอร์มซ่อนช่องตอนเลือกเงินสด, หน้าจ่ายเงินเดือนโชว์เป็น alert). ไม่มี validation รูปแบบเลขบัญชี — ธนาคารไทยฟอร์แมตไม่เหมือนกัน
- **(2026-08-18) Balance ทุก notification ผ่าน `calc.balance_snapshot(emp_id)`** — อ่าน employee row เอง แล้วเลือกโมเดลตาม `employment_status`/`holiday_mode`: `probation` → `{"mode":"probation"}` (LINE ไม่โชว์ block เลย), `monthly` → `compute_monthly_leave_balance` จาก `anchor_of(emp)` (accrued/used/remaining, **ไม่มีตัวเงิน** เพราะวันลาเกินโควตาหักตอนจ่ายผ่าน `deduction_days` อยู่แล้ว), `sunday` → `compute_overall_balance`. `_anchor_of` ย้ายไป `calc.anchor_of` (main.py import กลับมาใช้ชื่อเดิม). เลิกเรียก `compute_overall_balance` ตรงๆ ใน `line_notify` — เดิมทำให้แม่บ้าน monthly mode ได้ข้อความ "ยอดค้าง" ทั้งที่โควตาลายังเหลือ. i18n key ใหม่: `balance_monthly`, `monthly_leave`
