...
| Synology Reverse Proxy | `https://…:15055` → `http://localhost:5055` |
| Router port forward | External `15055` → NAS `15055` |
...
- For `transfer`, the employee form also takes **bank name + account number + account holder name** (`bank_name`, `bank_account`, `bank_account_name`; hidden while the method is `cash`) and the payment page shows them so whoever transfers can read the account off the screen
...
- Reminders are silently skipped if `MAID_LINE_CHANNEL_ACCESS_TOKEN` / `MAID_LINE_GROUP_ID` are not set
...
**Requirements:**
- `MAID_LINE_CHANNEL_SECRET` env var (for signature verification)
...
ตั้งค่าทั้งหมดที่ **root `.env`** (ไฟล์เดียวสำหรับทุก stack) — ไม่ต้องมี `.env` ใน folder นี้

```bash
# จาก root ของโปรเจกต์
cp .env.example .env
# แก้ไขค่าในส่วน LINE และ Maid Tracker ใน .env
```
...
| `MAID_LINE_CHANNEL_ACCESS_TOKEN` | _(empty)_ | LINE Messaging API channel token — leave blank to disable |
| `MAID_LINE_GROUP_ID` | _(empty)_ | LINE group ID (starts with `C`) — see [LINE Group Setup](#line-group-setup) |
| `MAID_LINE_CHANNEL_SECRET` | _(empty)_ | LINE channel secret for webhook signature verification — see [Webhook Setup](#webhook-setup) |
...
Add to root `.env` (at repo root — ไฟล์เดียวสำหรับทุก stack):
```env
MAID_LINE_CHANNEL_ACCESS_TOKEN=<your token>
...
**Current setup:** Synology Reverse Proxy handles HTTPS termination at `https://<NAS_HOST>:15055` → `http://localhost:5055`. No extra containers or tunnels needed. Router forwards external port **15055 → NAS port 15055**.
...
### 3 — Add MAID_LINE_CHANNEL_SECRET to root `.env`

Find the **Channel secret** in LINE Developers Console → **Basic settings** tab.

```env
MAID_LINE_CHANNEL_SECRET=abc123...
```
...
- **แจ้งเตือนแสดงความยินดีผ่าน LINE เมื่อกดผ่านโปร** — ข้อความไทย + บล็อกแปลภาษาแม่บ้าน (`notify_language`) บอกวันเริ่มเงินเดือนประจำ, เงินเดือน, รอบจ่าย (2 รอบ/รอบเดียว), วันหยุดที่จะได้ และย้ำว่าช่วงที่เหลือของเดือนยังจ่ายรายวัน
...
- ข้ามการแจ้งเตือนเงียบๆ ถ้ายังไม่ได้ตั้งค่า `MAID_LINE_CHANNEL_ACCESS_TOKEN` / `MAID_LINE_GROUP_ID`
...
| คำ | ผลลัพธ์ |
|----|---------|
| `ยอดสะสม` / `เช็คยอด` / `ยอดลา` / `แสดงยอด`| บอทตอบยอดสะสมปัจจุบัน |
...
**สิ่งที่ต้องมี:**
- env var `MAID_LINE_CHANNEL_SECRET` (สำหรับตรวจสอบ signature)
...
การแจ้งเตือนเป็น **opt-in** — ถ้าไม่ได้ตั้งค่า `MAID_LINE_CHANNEL_ACCESS_TOKEN` หรือ `MAID_LINE_GROUP_ID` ใน environment จะข้ามการแจ้งเตือนทั้งหมด โดยไม่กระทบการทำงานของแอป ข้อความถูกส่งเข้า **กลุ่ม LINE** ทำให้ทุกคนในกลุ่มเห็นพร้อมกันโดยใช้ API call เดียว ดูรายละเอียดที่ [ตั้งค่ากลุ่ม LINE](#ตั้งค่ากลุ่ม-line) ด้านล่าง
...
| `MAID_LINE_CHANNEL_ACCESS_TOKEN` | _(ว่าง)_ | LINE Messaging API channel token — เว้นว่างเพื่อปิดการแจ้งเตือน |
| `MAID_LINE_GROUP_ID` | _(ว่าง)_ | Group ID กลุ่ม LINE (ขึ้นต้นด้วย `C`) — ดู [ตั้งค่ากลุ่ม LINE](#ตั้งค่ากลุ่ม-line) |
| `MAID_LINE_CHANNEL_SECRET` | _(ว่าง)_ | Channel secret สำหรับตรวจสอบ webhook signature — ดู [ตั้งค่า Webhook](#ตั้งค่า-webhook) |
...
เพิ่มใน root `.env` (ที่ root ของ repo — ไฟล์เดียวสำหรับทุก stack):
```env
MAID_LINE_CHANNEL_ACCESS_TOKEN=<token ของคุณ>
MAID_LINE_GROUP_ID=C1a2b3c4d5...
```
...
**Setup ปัจจุบัน:** ใช้ Synology Reverse Proxy จัดการ HTTPS ที่ `https://<NAS_HOST>:15055` → `http://localhost:5055` ไม่ต้องมี container หรือ tunnel เพิ่มเติม Router forward port ภายนอก **15055 → NAS port 15055**
...
2. Tab **Messaging API settings** → **Webhook URL** → ตั้งค่าเป็น:
   ```
   https://<NAS_HOST>:15055/webhook/line
   ```
3. กด **Verify** — ต้องได้รับ `200 OK`
4. เปิด toggle **Use webhook**

### ขั้นที่ 3 — เพิ่ม MAID_LINE_CHANNEL_SECRET ใน root `.env`
