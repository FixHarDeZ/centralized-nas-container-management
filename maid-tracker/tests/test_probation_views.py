...
def _mk_probation(client, start="2025-02-01"):
    ...
    r = client.post("/api/employees", json={
        "name": "P", "start_date": start, "monthly_salary": 15600,
...
def test_probation_summary_no_holiday_no_monthly(client):
    ...
    client.post(f"/api/employees/{eid}/attendance", json={"work_date": "2025-02-05", "status": "leave"})            # -1.0
    client.post(f"/api/employees/{eid}/attendance", json={"work_date": "2025-02-06", "status": "leave", "half_day": True})  # -0.5
