...
- **Context ranking** — `_rank_context` และ `_fallback_scan` ให้น้ำหนัก title match ×10 เพื่อให้ page ที่ชื่อตรงกับ query เสมอ rank สูงกว่า daily log ที่มี keyword เยอะใน body
- **History pollution guard** — "ไม่พบข้อมูลใน Notion" response จะ**ไม่ถูกบันทึกลง history** เพื่อป้องกัน LLM ตอบซ้ำ pattern "ไม่พบ" ในคำถามเดิมครั้งต่อไป
