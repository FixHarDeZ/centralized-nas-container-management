...
def test_patch_expiry(client):
    ...
    conn.close()
    r = client.patch(
        "/api/prices/openai/gpt-4o/expiry",
        json={"expires_at": "2025-12-31"},
    )
    assert r.status_code == 200
    data = r.json()
...
def test_patch_expiry_model_not_found(client):
    r = client.patch(
        "/api/prices/nonexistent/model/expiry",
        json={"expires_at": "2025-12-31"},
    )
    assert r.status_code == 404
...
def test_post_schedule_custom_sources_invalid_url_dropped(
    client,
    tmp_path,
    monkeypatch,
):
