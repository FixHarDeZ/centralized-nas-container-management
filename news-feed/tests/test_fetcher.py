...
def _make_entry(url, title="Test", published="2026-05-23T07:00:00"):
    e = MagicMock()
    e.get = lambda k, d="": {"link": url, "title": title, "published": published}.get(
        k,
        d,
    )
    return e
...
def test_fetch_all_inserts_new_articles(
    mock_parse,
    mock_summarize,
    tmp_path,
    base_config,
    ...
    from app.models import get_article
    from app.models import get_conn as _get_conn
