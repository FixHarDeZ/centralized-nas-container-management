...
- Rotated `KUMA_WEBHOOK_SECRET` before the WAN forward goes live: the old value was a guessable name+date string, and it is the *only* guard on the one WAN-reachable unauthenticated path (and it rides in a query param, so it appears in DSM's RP access log in cleartext). New value = `openssl rand -hex 16` (32 chars) via `sops set` on `stacks.ops_bot.kuma_webhook_secret`. Kuma has no API for notification config, so: stopped `uptime-kuma`, backed up `kuma.db` to `kuma.db.bak-rotate`, `UPDATE notification SET config = replace(config, 'secret=<old>', 'secret=<new>') WHERE name='ops-bot'`, restarted. Verified after redeploy: auth=200, hook_newsec=200, hook_oldsec=401, kuma=running.
- Cert note for the RP entry: do **not** issue a new cert — friendly-reminder's RP on 15066 already serves the same hostname, so just bind that existing cert to the 15070 entry. The `ReverseProxy_2979f8fa-…` cert was issued against the misspelled hostname and is dead weight.
