...
**⚠️ ชื่อเรื่องบน myhr เปลี่ยนได้** — uploader แก้ชื่อ torrent ทีหลัง (เติมคำโปรยไทยหน้าชื่อ) แต่ Download Station ยังถือชื่อตอนโหลด ทั้ง task title และ `uri` ที่เราเขียนลง watch folder. ดังนั้นทุกจุดที่หา task ต้องเทียบ **ทุกชื่อที่รู้จัก** ผ่าน `db.hr_title_variants(site_id, *titles)` (ชื่อที่ caller ส่งมา + ชื่อล่าสุดในตาราง `torrents` ของ site_id) แล้วส่งเป็น list เข้า `dsm.find_task(tasks, titles, torrent_names)` ซึ่งรับได้ทั้ง str และ list. caller ทั้งสาม (`check_vanished`, `check_cleared`, `hr_delete._resolve`) ต้องใช้ชุดเดียวกัน — แก้แค่ตัวเดียวแล้วแถวจะ match ผ่านแต่ไปตายที่ `del_failed` ซึ่ง terminal. อาการเดิมคือแจ้ง "ไม่มี task ใน Download Station — โหลดจาก client อื่น" ทั้งที่ task seeding อยู่ (2026-08-24, site 2407483)
