...
**เช็ค DS ก่อนถามลบ**: `check_vanished` เรียก `hr_fix.ds_tasks()` ครั้งเดียวต่อรอบ แล้ว `dsm.find_task()` ต่อแถว — เจอ = ถามลบ, ไม่เจอ = `cleared` + note ไม่ถาม (torrent ที่โหลดจากมือถือ/client อื่น ไม่มีไฟล์ของเราให้ลบ), DS ล่ม = ไม่ตัดสิน คา snapshot ไว้. guard นี้ delete-only ห้ามแตะ `hr.fix_candidates()`.
