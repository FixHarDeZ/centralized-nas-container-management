...
- ที่รู้สึกว่าไม่ถาม เพราะ **ถามได้เฉพาะรอบ 09:10 / 21:10** — ไฟล์ที่ครบ 24 ชม. ตอน 22:00 ต้องรอถึง 09:10 (ช้าได้ถึง ~12 ชม.) และรอบนึงถามได้ไม่เกิน 3 รายการ (`_MAX_PROMPTS_PER_ROUND`)
- ตรวจสด 2026-08-04 08:59: 4 แถว `warn` เห็นล่าสุด 33.1 ชม. `hr_fix_get` = None ทั้งหมด (รอบ 21:10 เมื่อคืนยังไม่ถึง 24 ชม. จึงถูกข้ามอย่างถูกต้อง) — ยิง `check_hr(force=True)` แล้วถาม Telegram 3 รายการทันที เหลืออีก 1 รายการไปรอบหน้า
...
- หัก credit ได้สูงสุด 13 ชม. (`_MAX_CREDIT_H` = 1 รอบ + jitter) — รอบที่ fetch พัง `check_vanished` return ก่อนถึง `hr_seen_snapshot` snapshot เลยค้าง ถ้าไม่ cap แล้วเว้นไป 2-3 วัน credit จะ ≥ 48 ชม. = ทุกแถวที่หายผ่านหมด และ guard `state == "hit"` ก็ช่วยไม่ได้เพราะมันอ่าน snapshot เก่าตัวเดียวกัน
...
- **แก้ regression ที่เพิ่งสร้าง**: การ raise per-id error ทื่อๆ ทำให้ `_do_delete` (ลบ task ก่อนแล้วค่อยลบไฟล์) ตกที่ `del_failed` แล้วไม่ได้ไปถึง `delete_path` ในเคสปกติมากคือ user ลบ task ทิ้งเองระหว่างที่ปุ่มยืนยันค้างใน Telegram หลายวัน = ไฟล์ค้าง + แถว terminal. เปลี่ยนเป็นเจอ per-id error แล้ว `list_tasks` ซ้ำ ถ้า id ไม่อยู่แล้วนับสำเร็จ (ตรงกับสิ่งที่ทั้ง caller ต้องการ) ยังอยู่ค่อย raise — เช็คจาก task list จริง ไม่เดาความหมายของ code 544.

### 2026-08-20 — H&R auto-fix: task ที่กด fix แล้วขึ้น Unknown/Downloading ค้าง
- **อาการที่ user เจอ**: 3 task ที่กด fix จาก Telegram ใน Download Station ขึ้น Download/Progress = `Unknown`, status `Downloading` ไม่จบ, บางอันค้าง `Checking` ยาว
- **ตรวจของจริงก่อนแก้** (`SYNO.DownloadStation.Task.list` `additional=detail,transfer` ผ่าน `dsm.Dsm` ในคอนเทนเนอร์):
  - dbid_212 / dbid_213 = `downloading`, `speed_download` 1.5–3 MB/s แต่ `size_downloaded` ค้าง 0 ตลอด (sample ห่างกัน 5 นาที)
  - dbid_211 = `hash_checking` ตั้งแต่ 07:15 ไม่ขยับ, ไม่มีโฟลเดอร์ payload ใน `/volume1/@download/211` เลย
  - `~/Torrents_Download` **ไม่มีไฟล์เดิมของทั้ง 3 เรื่อง** (มีแต่ของ 4 task เก่าที่ seed อยู่) = payload ถูกลบไปก่อนหน้าแล้ว
  - อ่าน `.part` จริง (`dd`+`od`): `vid_720p.mp4.part` มี mp4 `ftyp` header + nonzero ~66 MB ใน 64 MB แรก = **โหลดลงดิสก์จริง ข้อมูลไหลปกติ**
- **สรุปสาเหตุ (ฝั่ง DS ไม่ใช่โค้ดเรา)**: ทุก task ที่สร้าง**หลัง** OOM outage 2026-08-19 รายงาน `size_downloaded = 0` ตลอด ส่วน task เก่า (สร้างก่อน) รายงานเต็มปกติ → **DS นับ progress ไม่ขึ้นเลย = task ไม่มีวันจบ ไม่มีวันเริ่ม seed** ไม่ใช่แค่ UI สวยงามผิด: dbid_213 ขนาด 680 MB รันมา ~1 ชม. ที่ 1.3–3 MB/s (พอโหลดจบ 4-5 รอบ) ยังขึ้น `downloading` + `dl=0` อยู่ ทั้งที่ `.part` มีข้อมูลจริงและ mtime ขยับตลอด
- ปัจจัยซ้อน: ไฟล์เดิมของทั้ง 3 เรื่องถูกลบไปก่อนแล้ว DS จึงต้อง**โหลดใหม่ทั้งไฟล์** (2.4 GB) ก่อนถึงจะ seed ไม่ใช่ resume — ต่อให้ DS ปกติก็กินเวลานาน
- **ทางแก้ฝั่ง NAS (ต้องทำเอง)**: restart แพ็กเกจ Download Station จาก DSM (`synopkg restart DownloadStation` ต้อง root — `sudo -n` ของเราได้แค่ docker) แล้วให้ task hash check ใหม่; dbid_211 (`hash_checking` 0 byte) ลบ task ทิ้งได้เลยไม่มี payload ให้เสีย
- **บั๊กจริงที่เป็นของเรา — `check_cleared` re-prompt ไม่จบ**: `_FIX_GRACE_H = 24` ตีตรา `stalled` จากเวลาอย่างเดียว แล้ว `scan_and_prompt` ข้ามแค่ `pending/fixed/skipped` (ไม่มี `stalled`) → ไฟล์ที่ DS กำลังโหลดใหม่ 2.4 GB (เกิน 24 ชม. แน่ๆ) จะถูกถาม fix ซ้ำแล้วหย่อน .torrent ตัวเดิมเข้าไปอีก วนไปเรื่อยๆ กินแบนด์วิดท์ฟรี
  - แก้: ก่อนตีตรา `stalled` เรียก `ds_tasks()` + `dsm.find_task` — **เจอ task = ไม่ stalled** (กำลังโหลด payload ที่หายไปอยู่), DS ตอบไม่ได้ = ข้ามไว้รอบหน้า, ไม่มี task จริงเท่านั้นถึง `stalled` + ยิง Telegram บอกว่า fix ไม่ติด (เดิมเงียบสนิท)
- ข้อความ prompt เดิมสัญญาว่า "ให้ Download Station seed ต่อ" ทั้งที่ไฟล์อาจถูกลบไปแล้ว → เติมบรรทัดเตือนว่าถ้าไฟล์เดิมหาย DS จะโหลดใหม่ทั้งไฟล์ก่อน
- self-check ใน `hr_fix.py __main__` เพิ่มเคส: fixed + มี task ใน DS ต้องคง `fixed`, fixed + ไม่มี task ต้องเป็น `stalled` — รันในคอนเทนเนอร์ผ่าน (`hr_fix self-check OK`), deploy `-s torrentwatch` แล้ว
- **สถานะใหม่ `wedged`**: task อยู่ใน DS แต่ผ่านไป `_FIX_WEDGED_H = 120` ชม. ยังไม่ seed = ค้างฝั่ง DS (เคสนี้เป๊ะ) → ยิง Telegram บอกครั้งเดียวให้ไปดู/ลบ task เอง แล้วหยุด (อยู่ใน skip list ของ `scan_and_prompt` เพราะหย่อน .torrent ซ้ำแก้ไม่ได้ตราบที่ task ยังอยู่) — ไม่งั้นแถวจะค้าง `fixed` เงียบตลอดกาล ไม่ cleared ไม่ stalled ไม่มีใครรู้
- **ค้างไว้ให้ user**: restart Download Station + ลบ task dbid_211 (ดูด้านบน)

### 2026-08-21 — เจอสาเหตุจริง: ตัวนับ DS ตาย + ชื่อไฟล์เกิน 255 ไบต์ (auto-nudge)
- restart แพ็กเกจ Download Station แล้ว **อาการไม่หาย** — task ใหม่ยังขึ้น `Unknown`
- **พิสูจน์ด้วย pause+resume**: `dbid_214` ค้างที่ `dl=4,210,688` นิ่งสนิท 6 นาทีทั้งที่ speed ~1 MB/s → สั่ง pause แล้ว resume → DS `hash_checking` → **`seeding` 2,114,704,248 ครบทันที**. `dbid_215` เหมือนกัน: API รายงาน `dl=0` มาตลอด กด pause/resume เผยว่าโหลดไปแล้ว **979 MB**
- **สรุปกลไก**: ตั้งแต่ OOM outage 19/08 ตัวนับ live ของ DS ไม่อัปเดตเลย มันคำนวณใหม่เฉพาะตอน **hash check** เท่านั้น → task ที่โหลดครบแล้วไม่รู้ตัวว่าครบ = ไม่เคยพลิกเป็น `seeding` = H&R ไม่มีวันจบ. ข้อมูลบนดิสก์ครบตลอด ไม่เคยเสีย
- **ของใหม่ `dsm.nudge_task()`** = pause → `sleep 5` → resume (บังคับ hash check) และ `hr_fix.check_cleared` เรียก `nudge_task(task["id"])` แทนการ `continue` เฉยๆ ในเคส "มี task ใน DS แต่ยังไม่ seed หลังพ้น grace 24 ชม." — hash check ถูกกว่าปล่อยให้เลย deadline H&R; เกิน `_FIX_WEDGED_H` (120 ชม.) ค่อยยอมแพ้เป็น `wedged` เหมือนเดิม
- **เคสที่ nudge ช่วยไม่ได้ — ชื่อยาวเกิน 255 ไบต์**: `dbid_216` (= task 206/211 ตัวเดิม เรื่อง "คลิปหลุดมาใหม่ หนุ่มลายล้น…") ค้าง `hash_checking` ตลอด ไม่มีโฟลเดอร์ payload ถูกสร้างเลย. `transmissiond.log`: `ERR open-files.cc:183 Couldn't open '/var/services/download/211/คลิปหลุดมาใหม่ …'` — ชื่อโฟลเดอร์ในตัว torrent เป็นไทย ~130 ตัว × 3 ไบต์ ≈ 390 ไบต์ **เกินเพดาน 255 ไบต์ของ ext4/btrfs** = สร้างไฟล์ไม่ได้ตั้งแต่แรก (เพดานเดียวกับที่ `db.torrent_filename` cap ไว้ฝั่งเรา แต่ชื่อ *ภายใน* torrent เราคุมไม่ได้). torrent นี้โหลดบน NAS ไม่ได้ถาวร — ต้องลบ task ทิ้ง
- Verify: `python dsm.py` + `python hr_fix.py` self-check ในคอนเทนเนอร์ผ่าน (เพิ่ม assert ว่า task ที่ DS ยังมี ต้องโดน nudge ไม่ใช่ re-prompt), deploy แล้ว, `dbid_214` seeding อยู่จริง
- (แก้ตามรีวิว) ย้าย nudge **ออกมานอก grace 24 ชม.** — เดิม `dbid_215` ที่เพิ่ง fix เช้านี้จะไม่ได้ nudge อีก ~16 ชม. ทั้งที่อาจโหลดครบไปแล้วและรอแค่ hash check; ตอนนี้เจอ task = nudge ทุกรอบทันที ส่วนตราหน้า `stalled` (ไม่มี task) ยังกันด้วย 24 ชม. เหมือนเดิม (ย้ายเช็คลงไปในสาขานั้นแทน)

### 2026-08-24 — "ไม่มี task ใน Download Station" ทั้งที่มี (uploader เปลี่ยนชื่อเรื่องบน myhr)
- อาการ: 03:10 บอทแจ้ง `✅ พ้น Hit & Run แล้ว` ตามด้วย `📱 ไม่มี task ใน Download Station — โหลดจาก client อื่น` แต่ใน DS มี task `Anna Ralphs [Pornhub]` seeding อยู่จริง
- **หลักฐาน**: `hr_fixes.title` = `Anna Ralphs [PornHub]` → `torrent_filename()` = `Anna Ralphs [PornHub].torrent` = **ตรงกับ DS `uri` เป๊ะ** แต่ Telegram พิมพ์ชื่อ `สมควรเก็บเป็นอย่างยิ่ง Anna Ralphs [PornHub]` = `snap["title"]` จาก myhr.php (uploader แก้ชื่อเรื่องเพิ่มคำโปรยหลังจากเราโหลดไปแล้ว)
- **Root cause**: `check_vanished` ทำ `title = snap["title"] or fix["title"]` แล้วส่ง **ชื่อเดียว** เข้า `dsm.find_task` → uri เทียบ `สมควรเก็บ….torrent` ไม่ตรง, title เทียบก็ไม่ตรง (DS เก็บ `[Pornhub]` h เล็ก) → `None` → เขียนแถวว่า "โหลดจาก client อื่น" แล้ว `hr_seen_forget` = ไฟล์หลุดจากระบบถาวร ไม่มีคำถามลบอีก
- **แก้ที่ฟังก์ชันร่วม ไม่ใช่ที่ caller เดียว**: `db.hr_title_variants(site_id, *titles)` คืนทุกชื่อที่รู้จัก (ชื่อ myhr ปัจจุบัน + ชื่อใน `hr_fixes` + ชื่อล่าสุดในตาราง `torrents` ของ site_id นั้น) และ `dsm.find_task` รับ `str | list[str]` ทั้ง title และ torrent_name. caller ที่ต้องแก้พร้อมกัน: `check_vanished`, `check_cleared` และ **`hr_delete._resolve`** — ถ้าแก้แค่ตัวแรก แถวจะ match ผ่านแล้วไปตายที่ปุ่มลบเป็น `del_failed` (terminal) = แย่กว่าเดิม
- Test ก่อนแก้ (Phase 4): เพิ่มเคสจริงใน `dsm.py __main__` — task uri `Anna Ralphs [PornHub].torrent` + ค้นด้วยชื่อ myhr ใหม่ ต้องได้ `None` (ยืนยัน fail จริงในคอนเทนเนอร์: `TypeError: 'NoneType' object is not subscriptable`) แล้วผ่านเมื่อส่ง list. `hr_delete` self-check เดิม monkeypatch `find_task` ด้วย lambda ที่กลืนอาร์กิวเมนต์อะไรก็ได้ → เปลี่ยนเป็นฟังก์ชันที่ assert ว่าได้รับ list จริง ไม่งั้นครึ่งลบไม่ถูกทดสอบเลย
- Verify: `dsm` / `db` / `hr_delete` / `hr_fix` self-check ผ่านทั้ง 4 ในคอนเทนเนอร์, deploy แล้ว
- **Blast radius**: แถวที่มี note "ไม่มี task ใน Download Station" ทั้งหมด **30 แถว** — เช็คกับ DS ตอนนี้เหลือ `2407483` (Anna Ralphs) แถวเดียวที่ task ยังอยู่จริง (แถวเก่าตรวจย้อนหลังไม่ได้แล้ว task ถูกลบไปนานแล้ว จึงไม่รู้ว่าตอนนั้นตัดสินถูกหรือผิด). แก้แถวนี้ด้วยมือ: เขียน note ใหม่ + ยิง `hr_delete.prompt_delete()` ซ้ำ (สถานะ `del_asked`, ปุ่มแรกไม่ลบอะไร)
- ทิ้งไว้ทีหลัง: fallback เทียบ title แบบ case-insensitive (`[PornHub]` vs `[Pornhub]`) — รอบนี้ uri match ครอบคลุมแล้ว ไม่รวมเข้ามาเพื่อให้รู้ว่าครึ่งไหนแก้ได้จริง
